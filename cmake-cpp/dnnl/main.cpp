...
public:
  ...
  unsigned long microseconds() const {
    unsigned long cost =
        std::chrono::duration_cast<std::chrono::microseconds>(du).count();
  ...
  unsigned long seconds() const {
    unsigned long cost =
        std::chrono::duration_cast<std::chrono::seconds>(du).count();
    return std::max(0lu, cost);
  }

  unsigned long minutes() const {
    unsigned long cost =
        std::chrono::duration_cast<std::chrono::minutes>(du).count();
    return std::max(0lu, cost);
  }

  unsigned long hours() const {
    unsigned long cost =
        std::chrono::duration_cast<std::chrono::hours>(du).count();
    return std::max(0lu, cost);
  }
};
...
int matmul() {
  ...
  Coster coster;
  dnnl::memory::dims x_dims = {2, 700, 800}, y_dims = {2, 800, 900},
                     r_dims = {2, 700, 900};
  int x_size =
      std::accumulate(x_dims.begin(), x_dims.end(), 1, std::multiplies<int>());
  int y_size =
      std::accumulate(y_dims.begin(), y_dims.end(), 1, std::multiplies<int>());
  int r_size =
      std::accumulate(r_dims.begin(), r_dims.end(), 1, std::multiplies<int>());

  dnnl::memory::desc x_md = {x_dims, dnnl::memory::data_type::f32,
                             dnnl::memory::format_tag::any};
  dnnl::memory::desc y_md = {y_dims, dnnl::memory::data_type::f32,
                             dnnl::memory::format_tag::any};
  dnnl::memory::desc r_md = {r_dims, dnnl::memory::data_type::f32,
                             dnnl::memory::format_tag::any};
  ...
  dnnl::memory x_m(dnnl::memory::desc{x_dims, dnnl::memory::data_type::f32,
                                      dnnl::memory::format_tag::abc},
                   engine, x.data());
  dnnl::memory y_m(dnnl::memory::desc{y_dims, dnnl::memory::data_type::f32,
                                      dnnl::memory::format_tag::abc},
                   engine, y.data());

  std::cout << "set-memory: " << coster.lap().milliseconds() << "(ms)"
            << std::endl;
  coster.reset();
  ...
  std::cout << "reorder: " << coster.lap().milliseconds() << "(ms)"
            << std::endl;
  coster.reset();

  stream.wait();
  for (int i = 0; i < 100; ++i) {
    dnnl::matmul(matmul_pd).execute(stream, {{DNNL_ARG_SRC, x_m_required},
                                             {DNNL_ARG_WEIGHTS, y_m_required},
                                             {DNNL_ARG_DST, r_m_required}});
  }
  stream.wait();

  std::cout << "execute: " << coster.lap().milliseconds() << "(ms)"
            << std::endl;
  coster.reset();
...
int conv2d() {
  ...
  int x_size =
      std::accumulate(x_dims.begin(), x_dims.end(), 1, std::multiplies<int>());
  int w_size =
      std::accumulate(w_dims.begin(), w_dims.end(), 1, std::multiplies<int>());
  int r_size =
      std::accumulate(r_dims.begin(), r_dims.end(), 1, std::multiplies<int>());
  ...
  dnnl::memory::desc x_md = {x_dims, dnnl::memory::data_type::f32,
                             dnnl::memory::format_tag::any};
  dnnl::memory::desc w_md = {w_dims, dnnl::memory::data_type::f32,
                             dnnl::memory::format_tag::any};
  dnnl::memory::desc r_md = {r_dims, dnnl::memory::data_type::f32,
                             dnnl::memory::format_tag::any};
  dnnl::convolution_forward::desc conv_d(
      dnnl::prop_kind::forward_inference, dnnl::algorithm::convolution_direct,
      x_md, w_md, r_md, strides, dilates, paddings, paddings);
  dnnl::convolution_forward::primitive_desc conv_pd(conv_d, engine);

  std::cout << "init-descriptions: " << coster.lap().milliseconds() << "(ms)"
            << std::endl;
  coster.reset();
  ...
  dnnl::memory x_m(dnnl::memory::desc{x_dims, dnnl::memory::data_type::f32,
                   ...
                   engine, x.data());
  dnnl::memory w_m(dnnl::memory::desc{w_dims, dnnl::memory::data_type::f32,
                                      dnnl::memory::format_tag::oihw},
                   engine, w.data());
  dnnl::memory r_m(dnnl::memory::desc{r_dims, dnnl::memory::data_type::f32,
                                      dnnl::memory::format_tag::nchw},
                   engine, r.data());
  ...
  for (int i = 0; i < 100; ++i) {
    dnnl::convolution_forward(conv_pd).execute(
        stream, {{DNNL_ARG_SRC, x_m_required},
                 {DNNL_ARG_WEIGHTS, w_m_required},
                 {DNNL_ARG_DST, r_m_required}});
  }
  stream.wait();
...
int main(int argc, char *argv[]) {
  ...
  if (thread_num > omp_get_max_threads())
    throw std::runtime_error("use too many threads");
