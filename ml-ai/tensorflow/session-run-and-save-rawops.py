...
@_register_func
def stridedslice():
    x = tf.placeholder(tf.float32, [3, 4], "x")
    begin = tf.placeholder(tf.int64, [1], "begin")
    end = tf.placeholder(tf.int64, [1], "end")
    stride = tf.placeholder(tf.int64, [1], "stride")
    r = tf.raw_ops.StridedSlice(input=x, begin=begin, end=end, strides=stride)

    dirname = "stridedslice"
    x_numpy = np.zeros(shape=[3, 4], dtype=np.float32)
    begin_numpy = np.array([0], dtype=np.int64)
    end_numpy = np.array([0], dtype=np.int64)
    stride_numpy = np.array([1], dtype=np.int64)
...
def stringjoin():
    ...
    r = tf.raw_ops.StringJoin(inputs=[x, y], separator=",")
    dirname = "stringjoin"
    rmdir(dirname)
    ...
    with tf.Session() as sess:
        ...
        print(sess.run([r], feed_dict={"x:0": x_numpy, "y:0": y_numpy}))
