...
def replace_zip_map(graph, zip_map_node):
    ...
    zip_map_key, zip_map_value = (
        zip_map_node.output[0] + ":key",
        zip_map_node.output[0] + ":value",
    )
    squeeze_node = onnx.helper.make_node(
        "Squeeze", inputs=zip_map_node.input, outputs=[zip_map_value]
    )
    ...
    graph.output.insert(
        ...
        onnx.helper.make_tensor_value_info(
            zip_map_value, onnx.TensorProto.FLOAT, (len_zip_map,)
        ),
    )
    graph.output.insert(
        replaced_idx,
        onnx.helper.make_tensor_value_info(
            zip_map_key, onnx.TensorProto.INT64, (len_zip_map,)
        ),
    )
    key = onnx.numpy_helper.from_array(
        np.array(zip_map_node.attribute[0].ints), zip_map_key
    )
