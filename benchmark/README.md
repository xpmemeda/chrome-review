...
- `--num-requests-sweep`：每个并发点统计多少个请求；单个值会应用到所有并发点，列表或 range 需要和 `--concurrency-sweep` 等长。
...
- `--num-requests-sweep`：每个 QPS 点统计多少个请求；单个值会应用到所有 QPS 点，列表或 range 需要和 `--qps-sweep` 等长。
...
`--qps-sweep` 和 `--concurrency-sweep` 都支持逗号列表和闭区间 range：
...
--num-requests-sweep 20:100:20
--concurrency-sweep 1,2,4,8
--concurrency-sweep 1:16:3
```
...
- `--num-prompt-tokens`：每个请求的近似 prompt token 数，必填。
- `--prompt-prefix-hit-rate`：prompt 前缀复用比例，范围 `[0, 1]`；synthetic dataset 会把这部分共享前缀作为独立的 `system` message。
- synthetic prompt 会默认加入短输出长度指令，目标为 `--min-tokens` 或 `--max-tokens`；该指令会计入 `--num-prompt-tokens` 的 suffix 预算。
- `--text-seed`：`-d synthetic-txt` 的 prompt seed，默认 `0`。

VLM synthetic 额外参数：

- `--image-width`：合成图片宽度，`-d synthetic-vlm` 必填。
- `--image-height`：合成图片高度，`-d synthetic-vlm` 必填。
- `--image-seed`：合成图片的基础随机种子；不传时随机生成。
...
- `--omni-image-cache`：图片 URL 到 base64 的缓存文件，默认读取 `~/workspace/ocean/service_shell/benchmark/round1_image_base64_cache.json`；缓存不存在时保留原 URL。
- `--omni-noise-bytes`：每个请求对最后一张命中缓存的图片追加的随机字节数，默认 `128`。
- `--omni-seed`：控制 app list shuffle、图片噪声和 user 后缀的随机种子，默认 `0`。

`-d omni-multi-message` 会读取模板中的 `messages` 和 `tools` 等额外顶层字段。每个请求会生成独立 `user`，改写倒数第二条 user 消息里的 `The available app list` 顺序，并把命中缓存的图片 URL 替换成 base64；最后一张图片会额外追加随机噪声。`model`、`max_tokens`、`temperature`、`top_p` 仍由 benchmark CLI 参数控制。
...
每行包含 `label`、`client_idx`、`req_idx`、延迟、本地/服务端输出 token 数、输出字符数、错误信息和完整 `output` 文本。
