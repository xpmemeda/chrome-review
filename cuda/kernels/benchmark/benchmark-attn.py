...
class Benchmark:
    ...
    def getContextLens(self, num_seqs, max_context_len, random_len):
        if random_len:
            context_lens = [
                random.sample(range(max_context_len + 1), 1) for _ in range(num_seqs)
            ]
        else:
...
class MultiBenchmark:
    ...
    def doBench(self):
        for i, num_seqs in enumerate(self.batch_list):
            x = []
            y = []
            for j, context_len in enumerate(self.context_list):
                print("num_seqs %d context_len %d" % (num_seqs, context_len))
                benchmark = Benchmark(
                    num_seqs,
                    self.num_heads,
                    self.num_heads_kv,
                ...
                x_ = benchmark.benchKernel(
                    "paged_attention_v1_1", Layout_K.VLLM, Layout_V.VLLM
                )
                y_ = benchmark.benchKernel(
                    "paged_attention_v1_8", Layout_K.CR, Layout_V.CR
                )
                x.append(x_)
                y.append(y_)

            plt.subplot(len(self.batch_list), 1, i + 1)
            plt.plot(
                self.context_list,
                x,
                label="paged_attention_v1_1",
                marker="o",
                color="b",
            )
            plt.plot(
...
if __name__ == "__main__":
    ...
    """
    # use_v1 = context_len <= 512 or num_seqs * num_heads > 256
    num_seqs = 4
    num_heads = 32
    num_heads_kv = 8
    head_size = 128
    ...
    multi_benchmark = MultiBenchmark(
        [1, 4, 8, 16],
        [16, 256, 1024],
        num_heads,
        num_heads_kv,
