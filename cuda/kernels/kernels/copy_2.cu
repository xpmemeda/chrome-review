...
__global__ void copy_2(  //
  ...
  for (int idx_h = warp; idx_h < h; idx_h += NUM_WARPS) {
    ...
    // NOTE: The more bytes a thread loads at a time, the faster the overall loading speed.
    //       Test on A10:
    //       uint16_t vs. uint32_t vs. uint4: 12.9ms vs. 10.2ms vs. 9.1ms
...
void copy_2_launcher(torch::Tensor& dst, const torch::Tensor& src) {
  ...
  copy_2<1024, 128><<<grids, 128>>>(dst_ptr, src_ptr, stride_n, stride_c, stride_h, 1024, 1024);
}

static cr::Register _([](pybind11::module& m) { m.def("copy_2", &copy_2_launcher); });
