...
#pragma unroll
    for (int i = 0; i < NUM_LOADS; ++i) {
      ...
      LOAD_T v = *reinterpret_cast<const LOAD_T*>(src_ptr + offset);
      // NOTE: 6.18 -> 6.42 The overhead of __shfl is small.
      v = __shfl_xor_sync(uint32_t(-1), v, WARP_SIZE / 2);
      v = __shfl_xor_sync(uint32_t(-1), v, WARP_SIZE / 2);
      *reinterpret_cast<LOAD_T*>(dst_ptr + offset) = v;
    }
  }
}

void copy_4_launcher(torch::Tensor& dst, const torch::Tensor& src) {
  ...
  copy_4<1024, 128><<<grids, 128>>>(dst_ptr, src_ptr, stride_n, stride_c, stride_h, 1024, 1024);
}

static cr::Register _([](pybind11::module& m) { m.def("copy_4", &copy_4_launcher); });
