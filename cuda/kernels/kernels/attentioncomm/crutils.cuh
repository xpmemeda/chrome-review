...
struct GemmPvAndStoreO_CR2 {
  ...
  using vvec_t = uint4;
  static constexpr int V_VEC_SIZE = sizeof(vvec_t) / sizeof(scalar_t);
  static constexpr int GROUP_SIZE = HEAD_SIZE / V_VEC_SIZE;
  static constexpr int NUM_GROUPS = WARP_SIZE / GROUP_SIZE;
  static_assert(NUM_GROUPS == 1 || NUM_GROUPS == 2 || NUM_GROUPS == 4);
...
#pragma unroll
      for (int iter = 0; iter < NUM_ITERS_PER_BLOCK; ++iter) {
        ...
        float token_logit = logits[token_idx];
        const int offset1 = physical_block_offset * HEAD_SIZE;
        const int offset2 = group_offset * V_VEC_SIZE;
        vvec_t v = *reinterpret_cast<const vvec_t*>(v_ptr + offset1 + offset2);
        cr_fma<vvec_t, V_VEC_SIZE>(accs_x_2, token_logit, v);
      }
    }
...
#pragma unroll
      for (int i = 0; i < V_VEC_SIZE; ++i) {
        accs_x_2[i] += __shfl_xor_sync(uint32_t(-1), accs_x_2[i], mask);
      }
    }

#pragma unroll
    for (int i = 0; i < N; ++i) {
      accs[i] = accs_x_2[i + group_idx * N];
    }
  }
  ...
  inline __device__ void storeO() {
    ...
    if (warp_idx == 0) {
      ...
      const int offset2 = group_offset * V_VEC_SIZE;
      cr_arr_to_vec_t(*reinterpret_cast<vec_t*>(out_ptr + offset1 + offset2), accs);
    }
  }
