...
__global__ void paged_attention_v2_kernel_8(   //
  ...
  using LoadQAndGemmQk = cr::LoadQAndGemmQk_CR1<scalar_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>;
  ...
  if (threadIdx.x == 0) {
    ...
    // printf("v2_8: qk_max %f exp_sum %f partition_idx %d\n", qk_max, exp_sum, partition_idx);
...
template <typename scalar_t, int HEAD_SIZE, int NUM_THREADS,
    int PARTITION_SIZE>
__global__ void paged_attention_v2_reduce_kernel_8(scalar_t* __restrict__ out,  // [num_seqs, num_heads, head_size]
...
template <int BLOCK_SIZE, int HEAD_SIZE, int NUM_THREADS, int PARTITION_SIZE>
void single_query_cached_kv_attention_launcher(  //
    cr::Tensor& out,                             // [num_seqs, num_heads, head_size]
    const cr::Tensor& query,                     // [num_seqs, num_heads, head_size]
    const cr::Tensor& key_cache,                 // [num_blocks, num_heads_kv, block_size, head_size]
  ...
  auto kernel = &paged_attention_v2_kernel_8<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, PARTITION_SIZE>;
  cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
  kernel<<<grid, block, shared_mem_size, stream>>>(exp_sums_ptr, max_logits_ptr, tmp_out_ptr, query_ptr, key_cache_ptr,
      value_cache_ptr, num_heads_kv, scale, block_tables_ptr, context_lens_ptr, max_num_blocks_per_seq,
      alibi_slopes_ptr, q_stride, kv_block_stride, kv_head_stride);
  paged_attention_v2_reduce_kernel_8<T, HEAD_SIZE, NUM_THREADS, PARTITION_SIZE>
...
void single_query_cached_kv_attention_launcher(  //
  ...
  if (num_seqs * num_heads * num_partitions > 256) {
    constexpr int NUM_THREADS = 128;
    single_query_cached_kv_attention_launcher<BLOCK_SIZE, HEAD_SIZE, NUM_THREADS, PARTITION_SIZE>(
...
static cr::Register _(
    [](pybind11::module& m) { m.def("paged_attention_v2_8", &decode_attn_6, "Layout_K.CR; Layout_V.CR"); });
