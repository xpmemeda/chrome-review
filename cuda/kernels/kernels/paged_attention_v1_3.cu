...
template <typename scalar_t, int HEAD_SIZE, int BLOCK_SIZE,
    int NUM_THREADS>
__global__ void single_query_cached_kv_attention_kernel_3(  //
...
#pragma unroll
  ...
  constexpr int NUM_LOADS = HEAD_SIZE / WARP_SIZE;
  float accs[NUM_LOADS];
#pragma unroll
...
#pragma unroll
      for (int load_idx = 0; load_idx < NUM_LOADS; ++load_idx) {
        const int offset2 = WARP_SIZE * load_idx + lane;
        half v = *(v_ptr + offset1 + offset2);
        accs[load_idx] += token_logit * __half2float(v);
      }
    }
  }
...
void single_query_cached_kv_attention_launcher(  //
  ...
  switch (head_size) {
    case 64: {
      ...
      cudaFuncSetAttribute(vllm::single_query_cached_kv_attention_kernel_3<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>,
          cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
      vllm::single_query_cached_kv_attention_kernel_3<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>
...
static Register _([](pybind11::module& m) { m.def("paged_attention_v1_3", &decode_attn_3); });
