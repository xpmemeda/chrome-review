...
template <typename scalar_t, int HEAD_SIZE, int BLOCK_SIZE,
    int NUM_THREADS>
__global__ void single_query_cached_kv_attention_kernel_4(  //
    scalar_t* __restrict__ out,                             // [num_seqs, num_heads, head_size]
    const scalar_t* __restrict__ q,                         // [num_seqs, num_heads, head_size]
    const scalar_t* __restrict__ k_cache,                   // Layout.CR
    const scalar_t* __restrict__ v_cache,                   // Layout.VLLM
  ...
  const int aligned_context_len = (context_len + BLOCK_SIZE - 1) / BLOCK_SIZE * BLOCK_SIZE;
  Softmax::doSoftmax(logits, red_smem, qk_max, aligned_context_len);
...
void single_query_cached_kv_attention_launcher(  //
  ...
  switch (head_size) {
    case 64: {
      ...
      cudaFuncSetAttribute(vllm::single_query_cached_kv_attention_kernel_4<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>,
          cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
      vllm::single_query_cached_kv_attention_kernel_4<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>
...
static cr::Register _(
    [](pybind11::module& m) { m.def("paged_attention_v1_4", &decode_attn_4, "Layout_K.CR; Layout_V.VLLM"); });
