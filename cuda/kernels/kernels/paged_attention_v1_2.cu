...
template <typename scalar_t, int HEAD_SIZE, int BLOCK_SIZE,
    int NUM_THREADS>
__global__ void single_query_cached_kv_attention_kernel_2(  //
  ...
  auto load_q_and_gemm_qk = LoadQAndGemmQk(q, q_vecs, k_cache, block_table, logits, red_smem, seq_idx, head_idx,
      kv_head_idx, q_stride, kv_block_stride, kv_head_stride, context_len, num_blocks, scale, alibi_slope, 0);
  ...
  // if (threadIdx.x == 0) {
  //   printf("v1_2: qk_max %f exp_sum %f\n", qk_max, exp_sum);
  // }
  __syncthreads();
...
void single_query_cached_kv_attention_launcher(  //
  ...
  switch (head_size) {
    case 64: {
      ...
      cudaFuncSetAttribute(vllm::single_query_cached_kv_attention_kernel_2<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>,
          cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
      vllm::single_query_cached_kv_attention_kernel_2<T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>
...
static Register _(
    [](pybind11::module& m) { m.def("paged_attention_v1_2", &decode_attn_2, "Layout_K.VLLM; Layout_V.VLLM"); });
