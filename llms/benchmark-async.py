...
class trtllmBackend:
    def __init__(self, cmd_arguments):
        ...
        tokenizer = cmd_arguments.model
        self.batch_size = cmd_arguments.b
...
class wnrBackend:
    def __init__(self, cmd_arguments, model, tokenizer):
        import wnr
...
async def main(cmd_arguments):
    global logger
    logger = init_logger(cmd_arguments.log)
    ...
    backend_cls = backends[cmd_arguments.backend]

    logger.info(
        f"Benchmark start: "
        f"device={Device()}, "
        f"backend={cmd_arguments.backend}-{backend_cls.version()}, "
        f"model={cmd_arguments.model}, "
        f"num_coroutine={cmd_arguments.b}, "
        f"num_prompt_tokens={cmd_arguments.i}, "
        f"num_prompt_hit_tokens={cmd_arguments.h}, "
        f"num_output_tokens={cmd_arguments.o}"
    )
    if cmd_arguments.f:
        requests_manager = RequestManagerParseFromSvrLog(
            AutoTokenizer.from_pretrained(cmd_arguments.model, trust_remote_code=True),
            cmd_arguments.n,
            cmd_arguments.f,
        )
    else:
    ...
    backend = backend_cls(cmd_arguments)

    metrics_managers = []
    coroutines = []
    for i in range(cmd_arguments.b):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--h", type=int, help="prefix caching hit tokens.", default=0)
    ...
    asyncio.run(main(cmd_arguments))
