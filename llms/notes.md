...
- 默认情况下，参数采用fp8加载，推理使用bf16。
- 集群类型：2机16卡H20
- 理论计算：
1. 每个token占用显存：hidden_size * element_size * 2(k&v) * num_layers = 7168 * 2 * 2 * 61 = 1708 KB
2. tp_size=16，每个token在每张卡上面占用显存：1708 KB / 16 = 106.75 KB
3. 671B模型，每张卡部署参数41B，参数类型fp8占用41G。剩余 55G * 0.9 = 50G. 可存放token数量 50 * 1024 * 1024 / 106.75 = 491 K （TODO MLA应该会更节省显存）

- 实际效果：
1. --mem-fraction-static 0.99 -> 789K OOM
2. --mem-fraction-static 0.95 -> 732K OOM
3. --mem-fraction-static 0.90 -> 660K OK
