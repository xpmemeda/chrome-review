...
if [ ! -d "${INSTALL_DIR}" ]; then
    ...
    mkdir build-shared && cd build-shared && ../configure --prefix=$INSTALL_DIR --enable-shared --enable-optimizations --with-openssl=$HOME/local/openssl-1.1.1w --with-openssl-rpath=auto && make -j && make install
    cd ..
    mkdir build-static && cd build-static && ../configure --prefix=$INSTALL_DIR --enable-optimizations --with-openssl=$HOME/local/openssl-1.1.1w --with-openssl-rpath=auto && make -j && make install
    cd ${INSTALL_DIR}
    ln -s lib lib64
    cd ${INSTALL_DIR}/bin
    ln -s python3 python
