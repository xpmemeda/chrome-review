...
void print_array_info(py::array ndarray) {
  std::cout << NPY_FLOAT16 << std::endl;
  std::cout << NPY_FLOAT32 << std::endl;
  std::cout << ndarray.dtype().num() << std::endl;
}
...
PYBIND11_MODULE(hello_world, m) {
  ...
  m.def("print_array_info", &print_array_info);
}