...
//
// NOTE: Maximum number of threads per block: 1024.
// NOTE: Number of 32-bit regular registers per multiprocessor: 64K. 所以说寄存器的总空间大小为256KB，比共享内存还大
// NOTE: Maximum number of 32-bit regular registers per thread: 256.
// NOTE: 也就是说，即使每个线程都用满256个寄存器，每个SM仍然可以支持256个线程（8个warp）工作。
//
// NOTE: Maximum number of resident blocks per multiprocessor: 16 ~ 32.
// NOTE: Maximum number of resident warps per multiprocessor: 32 ~ 64.
...
__global__ void softmax_cub_kernel(float* src, float* dst, int rows, int cols) {
  using BlockLoad = cub::BlockLoad<float, BLOCK_THREADS, ITEMS_PER_THREAD, cub::BLOCK_LOAD_STRIPED>;
  using BlockReduce = cub::BlockReduce<float, BLOCK_THREADS>;
  using BlockStore = cub::BlockStore<float, BLOCK_THREADS, ITEMS_PER_THREAD, cub::BLOCK_STORE_STRIPED>;
  ...
  src = src + blockIdx.x * cols;
  dst = dst + blockIdx.x * cols;

  float src_elements[ITEMS_PER_THREAD] = {-std::numeric_limits<float>::infinity()};
  // #pragma unroll
  //   for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
  //     int load_index = threadIdx.x + blockDim.x * i;
  //     if (load_index < cols) {
  //       src_elements[i] = src[load_index];
  //     } else {
  //       src_elements[i] = -std::numeric_limits<float>::infinity();
  //     }
  //   }
  BlockLoad(temp.load_storage).Load(src, src_elements, cols);

  float max_element = BlockReduce(temp.reduce_storage).Reduce(src_elements, cub::Max());
  __syncthreads();
  if (threadIdx.x == 0) {
    temp.x = max_element;
  }
...
#pragma unroll
  for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
    ...
    if (index < cols) {
      src_elements[i] = std::exp(src_elements[i] - max_element);
    } else {
      src_elements[i] = 0.f;
    }
  }

  float exp_sum = BlockReduce(temp.reduce_storage).Sum(src_elements);
  __syncthreads();
  if (threadIdx.x == 0) {
    temp.x = exp_sum;
  }
...
#pragma unroll
  for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
    src_elements[i] = src_elements[i] / exp_sum;
  }

  // #pragma unroll
  //   for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
  //     int store_index = threadIdx.x + blockDim.x * i;
  //     if (store_index < cols) {
  //       dst[store_index] = src_elements[i];
  //     }
  //   }
  BlockStore(temp.store_storage).Store(dst, src_elements, cols);
}
...
#define LAUNCH_SOFTMAX_KERNEL(NUM_ELEMENTS)                                   \
  case NUM_ELEMENTS: {                                                        \
    ...
    LAUNCH_SOFTMAX_KERNEL(2048);
    LAUNCH_SOFTMAX_KERNEL(4096);  // 每个线程用4096/32=128个寄存器。
    LAUNCH_SOFTMAX_KERNEL(8192);
    LAUNCH_SOFTMAX_KERNEL(16384);
    LAUNCH_SOFTMAX_KERNEL(32768);
    LAUNCH_SOFTMAX_KERNEL(65536);
#undef LAUNCH_SOFTMAX_KERNEL
    default: {
...
void softmax_v5(torch::Tensor& x, torch::Tensor& out) {
  unsigned rank = x.sizes().size();
  cr::cr_assert(rank >= 1, "%s:%d", __FILE__, __LINE__);
  cr::cr_assert(x.is_cuda(), "%s:%d", __FILE__, __LINE__);
  cr::cr_assert(out.is_cuda(), "%s:%d", __FILE__, __LINE__);
  ...
  float* out_ptr = reinterpret_cast<float*>(out.data_ptr());
  unsigned num_blocks = rows;
  unsigned cols_pow2 = 32;
  while (cols_pow2 < cols) {
    cols_pow2 *= 2;
  }
...
#define LAUNCH_KERNEL(N)                                                      \
  case N: {                                                                   \
    constexpr unsigned num_threads = N <= 1024 ? N : 1024;                    \
    constexpr unsigned items_per_thread = N / num_threads;                    \
    softmax_cub_kernel<num_threads, items_per_thread>                         \
        <<<num_blocks, num_threads, 0, stream>>>(x_ptr, out_ptr, rows, cols); \
    break;                                                                    \
  }
    LAUNCH_KERNEL(32);
...
static cr::Register _([](pybind11::module& m) {
  m.def("softmax_v1", &cr::softmax_v1);
  m.def("softmax_v2", &cr::softmax_v2);
  m.def("softmax_v3", &cr::softmax_v3);
  m.def("softmax_v4", &cr::softmax_v4);
  m.def("softmax_v5", &cr::softmax_v5);
});
