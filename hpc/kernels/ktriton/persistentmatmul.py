...
def validate(M, N, K, dtype):
    ...
    tma_persistent_result = None
    descriptor_persistent_result = (
        matmul_descriptor_persistent(a, b) if supports_tma() else None
    )
    ...
    if descriptor_persistent_result is not None:
        print(
            f"Tensor descriptor persistent: {naive_vs_descriptor_persistent} ", end=""
        )
    print()
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_arg="provider",
        line_vals=["matmul", "persistentmatmul", "torch"],
        line_names=["Matmul", "PersistentMatmul", "Torch"],
...
def benchmark(N, provider):
    a = torch.randn((N, N), device="cuda", dtype=torch.float16).to(dtype)
    b = torch.randn((N, N), device="cuda", dtype=torch.float16).to(dtype)
    b = b.T.contiguous()
    quantiles = [0.5, 0.2, 0.8]
    if provider == "matmul":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul(a, b.T), quantiles=quantiles
        )
    if provider == "persistentmatmul":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul_persistent(a, b.T), quantiles=quantiles
        )
    if provider == "torch":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: torch_matmul(a, b), quantiles=quantiles
        )
...
def show_profile(precision, profile_name):
    ...
    with open(file_name, "r") as f:
        gf, raw_metrics, device_info = proton_viewer.get_raw_metrics(f)
    proton_viewer.parse(
        metrics, file_name, include=None, exclude=None, threshold=None, depth=100
    )
    print(raw_metrics)
