...
# 可选：AGENT_PATH=/opt/tiger/remote-agent/agent.py TMUX_SESSION=remote-agent
set -Eeuo pipefail

AGENT_PATH=${AGENT_PATH:-/opt/tiger/remote-agent/agent.py}
AGENT_URL=${AGENT_URL:-https://raw.githubusercontent.com/xpmemeda/chrome-review/dev/codex/mcps/remote-fleet/agent.py}
TMUX_SESSION=${TMUX_SESSION:-remote-agent}
export AGENT_PATH AGENT_URL TMUX_SESSION

configure_proxy() {
    if [[ -n ${BERNARD_CONFIG+x} ]] || env | grep '^[^=]*MERLIN[^=]*=' >/dev/null; then
...
if [[ ${1:-} == --worker ]]; then
    ...
    stage=$(mktemp "${AGENT_PATH}.download.XXXXXX")
    # 仅下载 agent.py，先校验语法，再原子替换目标文件。
    network curl -fsSL "$AGENT_URL" -o "$stage"
    "$PYTHON" -m py_compile "$stage"
    mv -f "$stage" "$AGENT_PATH"
    exec "$PYTHON" "$AGENT_PATH" --root /
fi
